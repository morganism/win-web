...
function makeWindow(icon, title, frameurl, taskbar, res, wid, hei, webrowser=false) { /* IT WORKS!!! What's the catch? */
	window_set++;
	...
	if(webrowser){
		tmpfrm = $(document.createElement('iframe')).attr({
			id: "win_window_" + window_set + "_iframe",
			class: "win_window_iframe",
			allowtransparency: "true",
			sandbox: "allow-forms allow-modals allow-orientation-lock allow-pointer-lock allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-presentation allow-same-origin allow-scripts",
			allow: "keyboard-map",
			target: "_self",
			src: "./system/browser/browser.htm?urlparam=" + frameurl
		});
	}
	else
	{
		tmpfrm = $(document.createElement('iframe')).attr({
			...
			allow: "keyboard-map", // fixes some games but it is not implemented yet by browsers, but in future it will (2022+)
			target: "_self",
			src: frameurl
		});
	};
	
	//refocus on mouseover!
	tmpfrm[0].addEventListener('mouseover', function() { tmpfrm.focus(); console.log("refocus on iframe" + window_set); }, false);
	tmpfrm.appendTo( window_id );
	
	//var asf = document.getElementById("win_window_" + window_set + "_iframe");
	/*
	document.getElementById('win_window_" + window_set + "_iframe').addEventListener('mouseover',function(){
   myConfObj.iframeMouseOver = true;
  });
  
  document.getElementById('win_window_" + window_set + "_iframe').addEventListener('mouseout',function(){
      myConfObj.iframeMouseOver = false;
  });
*/
	//asf.onclick = alert("t");
	//.contentDocument.addEventListener('click', function(event) {alert(this.id);}, false);
...
function windowCenter(num) {
	...
	var winStyle = "top: " + (topHeight - 20) + "px;left: " + leftWidth + "px;";
